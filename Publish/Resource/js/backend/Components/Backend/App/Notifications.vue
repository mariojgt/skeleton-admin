<template>
    <div class="dropdown dropdown-end">
        ...
        <button class="btn btn-ghost btn-circle relative" aria-label="Notifications">
            ...
            <div v-if="notificationCount > 0" class="absolute -top-1 -right-1 flex items-center justify-center">
                <div class="badge badge-primary badge-sm">{{ notificationCount }}</div>
            </div>
        </button>

        <!-- Dropdown Content -->
        <div tabindex="0" class="dropdown-content z-30 shadow-lg menu bg-base-100 rounded-box w-96 overflow-hidden">
            <!-- Notifications List -->
                        ...
                        <div class="flex items-start p-4 hover:bg-base-200 transition-colors duration-200">
                            ...
                            <div class="flex-shrink-0">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center"
                                    ...
                                    <component :is="getNotificationIcon(notification.data.type)" class="w-5 h-5"
            ...
            <!-- Empty State -->
            <div v-else class="p-6 text-center text-base-content/70">
            ...
            <div class="border-t border-base-200">
                <button v-if="notificationCount > 0" @click="handleClearAll"
                ...
                <button v-else @click="handleRefresh"
                    class="flex items-center justify-center w-full gap-2 px-4 py-3 text-sm text-base-content/70 hover:bg-base-200 transition-colors duration-200">
...
const fetchNotifications = async () => {
    try {
        ...
        if (notificationCount.value > 0) {
            startWindToast('info', 'You have new notifications', 'info');
        }
    } catch (error) {
...
const handleClearAll = async () => {
    try {
        ...
        startWindToast('success', 'Notifications cleared', 'success');
    } catch (error) {
